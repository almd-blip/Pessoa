...
export default function Settings({ onResetAllData, onRestoreDemoData, defaultTab, accessibilitySettings, onAccessibilitySettingsChange }: SettingsProps) {
  ...
  const handleExportData = () => {
    const data = { profile: { name: scholarName, affiliation, areasOfInterest }, dailyFocus: localStorage.getItem('daily_focus') || '', smallWins: localStorage.getItem('wellbeing_small_wins') || '', draftText: localStorage.getItem('draft_companion_text') || '', projectType: localStorage.getItem('scholar_project_type') || '', accessibility: effectiveAccSettings, feedbackLogs: localStorage.getItem('scholar_feedback_logs') || '[]' };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `pessoa_backup_${Date.now()}.rcp`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url); triggerToast('Backup downloaded successfully.');
  };
  return (
    <div className="w-full space-y-6 font-sans text-left pb-16" id="settings-module">
      ...
      {activeTab === 'ai' && <div className="animate-fadeIn text-left"><LocalAIRuntimeManager onConfigSaved={() => triggerToast('AI settings saved.')} /></div>}
      ...
      {activeTab === 'backup' && <div className="space-y-5 animate-fadeIn text-left"><div><h2 className="text-xl font-semibold text-[#1B0A3B] dark:text-stone-100">Backup & data</h2><p className="mt-1 text-sm text-stone-600 dark:text-stone-400">Keep a copy of your work or manage the data stored on this device.</p></div><div className="space-y-4 max-w-2xl"><div className="flex flex-wrap justify-between items-center gap-4 py-3 border-b border-stone-200 dark:border-stone-800"><div><p className="font-semibold text-sm text-stone-850 dark:text-stone-200">Storage used on this device</p><p className="text-xs text-stone-500 dark:text-stone-400 mt-1">Your notes, journal entries, feedback, and saved items stay on your device.</p></div><span className="font-mono text-xs text-stone-600 dark:text-stone-400">{Math.round(JSON.stringify(localStorage).length / 1024)} KB used</span></div><div className="flex flex-wrap gap-3"><button type="button" onClick={handleExportData} className="min-h-11 border border-stone-300 dark:border-stone-700 bg-transparent py-2.5 px-4 rounded-lg hover:bg-stone-50 dark:hover:bg-stone-900 transition-colors cursor-pointer text-sm font-medium text-stone-700 dark:text-stone-300">Download backup file (.rcp)</button>{onRestoreDemoData && <button type="button" onClick={() => { onRestoreDemoData(); triggerToast('Original demo projects, library, and milestones restored.'); }} className="min-h-11 border border-[#1D9E75]/50 text-[#1D9E75] py-2.5 px-4 rounded-lg hover:bg-[#1D9E75]/5 transition-colors cursor-pointer text-sm font-medium">Restore original demos</button>}<button type="button" onClick={() => { if (confirm('Are you sure you want to delete all saved data on this device? This cannot be undone.')) onResetAllData(); }} className="min-h-11 border border-red-200 text-red-700 py-2.5 px-4 rounded-lg hover:bg-red-50/50 transition-colors cursor-pointer text-sm font-medium">Delete all local data</button></div></div><div className="max-w-2xl pt-3 border-t border-stone-200 dark:border-stone-800 text-sm text-stone-600 dark:text-stone-400 space-y-1"><p className="font-semibold text-[#1B0A3B] dark:text-stone-200">Privacy & offline storage</p><p className="leading-relaxed">Pessoa keeps your data private on your own device. We do not store your data on external servers or track your activity.</p></div></div>}
    </div>
  );
}
