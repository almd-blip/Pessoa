...
export default function GuidedAISetup() {
  ...
  return (
    <div className="w-full space-y-8 text-left font-sans">
        ...
        <div className="space-y-2">
          {[
            ...
            <label key={id} className="flex items-center gap-3 cursor-pointer py-1.5">
              <input type="radio" name="device" value={id} checked={device === id} onChange={() => { setDevice(id as Device); setAndroidVersion(null); setShowResult(false); }} className="h-4 w-4" style={{ accentColor: teal }} />
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {[
              ...
              <label key={id} className="flex items-center gap-3 cursor-pointer py-1.5">
                <input type="radio" name="android-version" value={id} checked={androidVersion === id} onChange={() => { setAndroidVersion(id as AndroidVersion); setShowResult(false); }} className="h-4 w-4" style={{ accentColor: teal }} />
                <span className="text-sm" style={{ color: indigo }}>{label}</span>
              </label>
            ))}
          </div>
      ...
      {ready && !showResult && (
        <button type="button" onClick={() => setShowResult(true)} className="min-h-11 rounded-lg px-5 py-2.5 text-sm font-semibold text-white transition-colors" style={{ backgroundColor: burgundy }}>Show my setup</button>
      )}
      ...
      {showResult && ready && (
        <div className="space-y-6 border-t border-stone-200 dark:border-stone-800 pt-6">
          ...
          {route === 'app' ? (
            ...
            <>
              <SetupStep number={1} title="Check your browser"><p>Use a recent browser. Pessoa will check whether your device can use browser AI.</p></SetupStep>
              ...
              <SetupStep number={3} title="Download the model"><p>Tap the download button. You need internet the first time. The model is saved on your device.</p></SetupStep>
