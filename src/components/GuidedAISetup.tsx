...
function LocalServerGuide({ expertise, device, androidVersion, browserReason }: { expertise: Expertise; device: Device; androidVersion: AndroidVersion | null; browserReason?: string | null }) {
  ...
  if (device === 'android' && androidVersion === 'older') {
    return <div className="space-y-5">
      ...
      <SetupStep number={2} title="Open Termux and give it access to your Downloads folder"><ol className="list-decimal space-y-2 pl-5"><li>Open <strong>Termux</strong>.</li><li>Enter this command:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs break-all">termux-setup-storage</code></li><li>Press <strong>Enter</strong>.</li><li>Android will ask whether Termux can access files on your device. Select <strong>Allow</strong>.</li></ol><p className="mt-3">Termux can now access your Android storage. Your Downloads folder is available at <code className="font-mono text-xs">~/storage/downloads</code>.</p></SetupStep>

      <SetupStep number={3} title="Install the AI engine"><ol className="list-decimal space-y-2 pl-5"><li>In Termux, enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">pkg update</code></li><li>Press <strong>Enter</strong> and wait for it to finish.</li><li>Then enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">pkg install llama.cpp</code></li><li>Press <strong>Enter</strong>.</li><li>If Termux asks you to confirm the installation, type <strong>y</strong> and press <strong>Enter</strong>.</li></ol></SetupStep>

      <SetupStep number={4} title="Download the AI model"><ol className="list-decimal space-y-2 pl-5"><li>Download the recommended <strong>Qwen 2.5 0.5B Instruct</strong> GGUF model to your Android <strong>Downloads</strong> folder.</li><li>Wait until the download has completely finished.</li><li>The model file should end in <code className="font-mono text-xs">.gguf</code>.</li></ol><p className="mt-3">For older or lower-memory Android devices, start with the small Qwen 2.5 0.5B model. It requires considerably less memory than larger models.</p></SetupStep>

      <SetupStep number={5} title="Check that Termux can see the model"><ol className="list-decimal space-y-2 pl-5"><li>Return to Termux.</li><li>Enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">ls -lh ~/storage/downloads</code></li><li>Press <strong>Enter</strong>.</li><li>Termux will display the files in your Downloads folder.</li></ol><p className="mt-3">Look for the model file ending in <code className="font-mono text-xs">.gguf</code>. The filename is shown at the end of its line, for example <code className="font-mono text-xs">qwen2.5-0.5b-instruct-q4_k_m.gguf</code>.</p><p className="mt-2">If the model is not listed, open Android's <strong>Files → Downloads</strong> and check that the download has finished. Then run the command again.</p></SetupStep>

      <SetupStep number={6} title="Start the AI server"><ol className="list-decimal space-y-2 pl-5"><li>Use the exact filename shown in Step 5.</li><li>Enter the following command, replacing the filename after <code className="font-mono text-xs">-m</code> with your actual model filename:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs break-all">llama-server -m ~/storage/downloads/qwen2.5-0.5b-instruct-q4_k_m.gguf --host 127.0.0.1 --port 8080</code></li><li>Press <strong>Enter</strong>.</li><li>Wait while llama-server loads the model.</li></ol><p className="mt-3">When the server is running, leave Termux open. It is now listening for Pessoa at <code className="font-mono text-xs">127.0.0.1:8080</code>.</p></SetupStep>

      <SetupStep number={7} title="Connect Pessoa"><ol className="list-decimal space-y-2 pl-5"><li>Return to Pessoa.</li><li>Open <strong>AI Settings</strong>.</li><li>Choose <strong>Custom / Local Server</strong>.</li><li>Enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">http://127.0.0.1:8080/v1</code></li><li>Enter the model name shown by your server if Pessoa asks for one.</li><li>Select <strong>Test connection</strong>.</li></ol><p className="mt-3">A successful test means Pessoa can send AI requests to the model running on your Android device.</p></SetupStep>
    </div>;
  }
  ...
  return <div className="space-y-5">
    ...
    <SetupStep number={2} title="Download and start a small model"><ol className="list-decimal space-y-2 pl-5"><li>Open a terminal or command prompt.</li><li>Enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">ollama run qwen2.5:0.5b</code></li><li>Press <strong>Enter</strong>.</li><li>Wait for the model to download and load.</li></ol><p className="mt-3">The Ollama server runs locally on your computer at <code className="font-mono text-xs">http://localhost:11434</code>.</p></SetupStep>
    <SetupStep number={3} title="Connect Pessoa"><ol className="list-decimal space-y-2 pl-5"><li>Return to Pessoa and open <strong>AI Settings</strong>.</li><li>Choose <strong>Custom / Local Server</strong>.</li><li>Enter:</li><li><code className="block rounded bg-stone-100 dark:bg-stone-900 p-2 font-mono text-xs">http://localhost:11434/v1</code></li><li>Enter the model name <code className="font-mono text-xs">qwen2.5:0.5b</code> if Pessoa asks for it.</li><li>Select <strong>Test connection</strong>.</li></ol></SetupStep>
  </div>;
}
